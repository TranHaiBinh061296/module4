    ...
    public void validate(Object target, Errors errors) {
        ...
        if (transactionAmount != null && transactionAmount.length() > 0) {
            if (transactionAmount.length() > 9){
                errors.rejectValue("transactionAmount", "transactionAmount.max", "Số tiền rút tối đa là 100.000.000 VNĐ");
                return;
            }

            if (transactionAmount.length() < 6){
                errors.rejectValue("transactionAmount", "transactionAmount.min.length", "Số tiền rút thấp nhất là 10.000 VNĐ");
                return;
            }
